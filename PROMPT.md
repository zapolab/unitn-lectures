...
## 4. Typst (una passata)
Segui `STYLE.md` per vincoli tecnici, layout, firme degli helper, pattern, struttura, lunghezza. Scrivi il `.typ` **in una sola passata completa**; poi correggi SOLO con edit mirati (sostituzione del frammento errato), **mai riscrivere il file intero** dopo un errore. In testa:
```typst
